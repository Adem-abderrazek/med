...
export class PatientService {
  ...
  async markMedicationTaken(patientId: string, reminderId: string): Promise<void> {
    try {
      ...
      // CRITICAL: Validate that the current time is at or after the scheduled time
      const now = new Date();
      const scheduledTime = new Date(reminder.scheduledFor);
      
      // Allow a 5-minute grace period before the scheduled time for early confirmation
