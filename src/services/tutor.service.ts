...
export interface PatientWithNearestMedication {
  ...
  reminderId: string;
  timeUntilMedication: number; // in minutes
}
...
class TutorService {
   ...
   */
  async getPatientsWithNearestMedications(tutorId: string): Promise<PatientWithNearestMedication[]> {
    try {
      // Get current time
      const now = new Date();
      ...
      const result: PatientWithNearestMedication[] = Array.from(patientNearestReminders.values())
        .map(reminder => {
   ...
   */
  async getMedicationAlerts(tutorId: string) {
    try {
   ...
   */
  async sendPatientInvitation(tutorId: string, patientData: {
    firstName: string;
    lastName: string;
    phoneNumber: string;
    audioMessage?: string;
    ...
    try {
      ...
      const smsResult = await smsService.sendInvitationSMS(
        ...
        generatedEmail || '',
        generatedPassword || 'Utilisez votre numéro de téléphone'
      );
   ...
   */
  async confirmMedicationManually(tutorId: string, reminderId: string) {
    try {
   ...
   */
  async searchPatients(tutorId: string, query: string) {
    try {
      const q = query.trim();
      if (!q) return [];
   ...
   */
  async getPatientProfile(tutorId: string, patientId: string) {
    try {
   ...
   */
  async getPatientAdherenceHistory(tutorId: string, patientId: string, daysBack: number = 30) {
    try {
      // Verify relationship - support both tuteur and medecin
      const relation = await prisma.userRelationship.findFirst({
        where: {
   ...
   */
  async createPrescriptionForPatient(tutorId: string, payload: {
    patientId: string;
    medicationName: string;
      ...
      for (const s of schedules) {
        ...
        await prisma.medicationSchedule.create({
          data: {
            ...
            daysOfWeek: s.days,
            scheduleType: (scheduleType as any) || 'daily',
   ...
   */
  async updatePrescriptionForPatient(tutorId: string, prescriptionId: string, payload: {
    medicationName?: string;
    medicationGenericName?: string;
    medicationDosage?: string;
    medicationForm?: string;
    ...
    try {
      ...
      const updatedPrescription = await prisma.prescription.update({
        ...
        data: {
          ...
          endDate: payload.endDate !== undefined ? (payload.endDate ? new Date(payload.endDate as any) : null) : prescription.endDate,
   ...
   */
  async deletePrescriptionForPatient(tutorId: string, prescriptionId: string) {
    try {
      // Verify prescription exists and belongs to a patient linked to this tutor
      const prescription = await prisma.prescription.findUnique({
        where: { id: prescriptionId },
   ...
   */
  async getAllPatientsForTutor(tutorId: string) {
    try {
      // Find all patients under this tutor or doctor (both relationship types)
      const relationships = await prisma.userRelationship.findMany({
        where: {
   ...
   */
  async getVoiceMessages(tutorId: string, patientId?: string) {
    try {
   ...
   */
  async createVoiceMessage(tutorId: string, data: { patientId: string; fileUrl: string; fileName?: string; title?: string; durationSeconds?: number }) {
    try {
   ...
   */
  async deleteVoiceMessage(tutorId: string, voiceMessageId: string) {
    try {
   ...
   */
  async deletePatient(caregiverId: string, patientId: string) {
    try {
