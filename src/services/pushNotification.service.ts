...
export class PushNotificationService {
   ...
   */
  private async sendFirebaseNotification(token: string, title: string, body: string, data?: any): Promise<boolean> {
    try {
   ...
   */
  private async sendExpoNotification(token: string, title: string, body: string, data?: any): Promise<boolean> {
    try {
  ...
  async sendMedicationReminder(
    patientId: string,
    reminderData: MedicationReminderData
  ): Promise<boolean> {
    try {
  ...
  async sendTestNotification(userId: string): Promise<boolean> {
    try {
      ...
      return await this.sendPushNotification(
        user.expoPushToken,
        '🧪 Test Notification',
        `Bonjour ${user.firstName}! Ceci est une notification de test.`,
        testData
      );
    } catch (error) {
