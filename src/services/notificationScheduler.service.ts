...
export class NotificationSchedulerService {
  ...
  private tutorAlertInterval: NodeJS.Timeout | null = null;
   ...
   */
  private async checkDueMedicationReminders(): Promise<void> {
    try {
   ...
   */
  private async checkTutorAlerts(): Promise<void> {
    try {
   ...
   */
  private groupRemindersByPatientAndTime(reminders: any[]): any[][] {
    const groups: any[][] = [];
   ...
   */
  private async sendGroupedMedicationReminder(reminderGroup: any[]): Promise<void> {
    try {
      ...
      if (patient.expoPushToken && patient.notificationsEnabled) {
        pushSuccess = await pushNotificationService.sendMedicationReminder(
          patient.id,
          reminderData
        );
   ...
   */
  private formatMedicationSMS(reminderGroup: any[], patientFirstName: string): string {
    if (reminderGroup.length === 1) {
      // Single medication
      const reminder = reminderGroup[0];
   ...
   */
  private async sendTutorAlertForReminder(reminder: any): Promise<void> {
    try {
      const reminderData = {
        reminderId: reminder.id,
        medicationName: reminder.prescription.medication.name,
      ...
      const success = await pushNotificationService.sendTutorAlert(
        reminder.patientId,
        reminderData
      );
   ...
   */
  async createTestReminder(patientId: string, delaySeconds: number = 10): Promise<string> {
    try {
