...
class TutorController {
     ...
     */
    async getPatientsWithNearestMedications(req, res) {
        try {
     ...
     */
    async sendPatientInvitation(req, res) {
        try {
     ...
     */
    async confirmMedicationManually(req, res) {
        try {
            const tutorId = req.user.userId;
    ...
    async createPrescription(req, res) {
        try {
            const tutorId = req.user.userId;
            const { patientId, medicationName, medicationGenericName, medicationDosage, medicationForm, medicationDescription, medicationImageUrl, customDosage, instructions, schedules, isChronic, endDate, scheduleType, intervalHours } = req.body;
    ...
    async updatePrescription(req, res) {
        try {
            ...
            const { medicationName, medicationGenericName, medicationDosage, medicationForm, medicationDescription, medicationImageUrl, customDosage, instructions, schedules, isChronic, endDate, scheduleType, intervalHours } = req.body;
            if (!prescriptionId) {
     ...
     */
    async uploadVoiceMessage(req, res) {
        try {
            ...
            const uploadResult = await firebaseStorageService.uploadVoiceFile(fileBase64, safeName, mimeType || 'audio/m4a');
     ...
     */
    async getVoiceMessages(req, res) {
        try {
            const tutorId = req.user.userId;
     ...
     */
    async deleteVoiceMessage(req, res) {
        try {
            const tutorId = req.user.userId;
            const { id } = req.params;
     ...
     */
    async generateTodaysReminders(req, res) {
        try {
     ...
     */
    async generateRemindersForNextDays(req, res) {
        try {
