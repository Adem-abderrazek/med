...
class DoctorService {
    ...
    async searchPatients(doctorId, query) {
        try {
            ...
            const filteredPatients = allPatients.filter(patient => patient.firstName.toLowerCase().includes(query.toLowerCase()) ||
    ...
    // Get patient profile
    async getPatientProfile(doctorId, patientId) {
        try {
    ...
    async addPatientMedication(doctorId, patientId, medicationData) {
        try {
            // Create medication
            const medication = await prisma.medication.create({
                data: {
