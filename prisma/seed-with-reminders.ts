...
import { PrismaClient, UserType, RelationshipType, User, Medication, Prescription, UserRelationship, ReminderStatus, AlertType, AlertStatus } from "@prisma/client";
...
async function main() {
  ...
  const patientRelationships: UserRelationship[] = [];
  ...
  const missedAlert1 = await prisma.alert.create({
    data: {
      ...
      alertType: AlertType.missed_medication,
